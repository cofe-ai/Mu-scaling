...
# for mup
from modeling.deepspeed_mup import deepspeed_init, process_param_groups
from transformers.optimization import get_scheduler
from modeling.mup_utils import get_linear_schedule_with_inverse_log_warmup
...
class MupTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                if self.args.exit_steps is not None and step > self.args.exit_steps:
                    exit()
                ...
                # works only when deepspeed is off
                if (self.state.global_step + 1) % self.args.logging_steps == 0:
                    for k, v in model.module.named_parameters():
                        # torch.abs(logits).mean(dtype=torch.float32).item()
                        if v.grad is not None:
                            self.grad_mean_scalar_dict[k] = torch.abs(v.grad).mean(dtype=torch.float32).item()
    ...
    ############ added for Mup grad check without deepspeed. ###############
    def create_optimizer(self):
        """
        This is triggered if deepspeed is off.
        Mup re-grouping of optimizer parameters.
        """
        ...
        if self.optimizer is None:
            ...
            print("use_mup:")
            print(self.args.use_mup)

            if self.args.use_mup:
                new_param_groups = []
                decoupled_wd = False
                for param_group in process_param_groups(optimizer_grouped_parameters, lr=self.args.learning_rate):
                ...
                optimizer_grouped_parameters = new_param_groups
    ...
    def create_scheduler(self, num_training_steps: int, optimizer: torch.optim.Optimizer = None):
        ...
        if self.lr_scheduler is None:
            if self.args.log_warmup:
                self.lr_scheduler = get_linear_schedule_with_inverse_log_warmup(
