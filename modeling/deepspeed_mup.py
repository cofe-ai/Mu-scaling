# This modifies transformers/deepspeed.py，Re-group parameters according to mup AdamW rules.
"""
...
def deepspeed_init(trainer, num_training_steps, resume_from_checkpoint=None, inference=False):
    ...
    if args.use_mup:
        ############# begin mup re-grouping #############
        model_parameters = dict(model.named_parameters())
        new_param_groups = []
        width_mult_group = []
        decoupled_wd = False
        matrix_like_key_words = ["c_attn.weight", "c_proj.weight", "c_fc.weight", "c_proj.weight"]
        for param_group in process_param_groups(model_parameters, lr=trainer.args.learning_rate):
            # param_group: {"params": dict<{name:params}>, "lr": float, "weight_decay": float}
            ...
            for n, p in param_group['params'].items():
                if not p.requires_grad:
                    continue
                if any([key in n for key in matrix_like_key_words]):
                    matrix_like_p[args.width_mult_for_weights]['params'].append(p)
                else:
            ...
            width_mult_group.extend(list(matrix_like_p.keys()) + [1.0])

        model_parameters = new_param_groups
    ...
    # Must reset all min_lrs, max_lrs, delta_lrs due to Deepspeed Implementation of Schedulers.
    # Temporarily, solve this with post-processing
    if args.use_mup:
        assert len(width_mult_group) == len(lr_scheduler.delta_lrs)
        for i in range(len(width_mult_group)):
            lr_scheduler.min_lrs[i] /= width_mult_group[i]
            lr_scheduler.max_lrs[i] /= width_mult_group[i]
            lr_scheduler.delta_lrs[i] /= width_mult_group[i]
    
    print(f"modified min_lrs:{lr_scheduler.min_lrs}, max_lrs:{lr_scheduler.max_lrs},\
          delta_lrs:{lr_scheduler.delta_lrs}")
